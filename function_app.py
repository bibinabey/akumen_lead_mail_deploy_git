...
def map_course_to_db(course_value: str) -> str:
    ...
    return normalized  # return processed value
# ── Webhook trigger ───────────────────────────────────────────────────────────
...
def webhook(req: func.HttpRequest) -> func.HttpResponse:
    ...
    logger.info(f"Webhook raw body: {json.dumps(body)}")
...
@app.queue_trigger(
    arg_name="msg",
    queue_name="contact-form-webhook-queue",   # must be the hard-coded queue name string here
...
def process_message(message_id: str, dequeue_count:int):
    ...
    crm_payload = {
        "name":          parsed.get("name") or "N/A",
        "mobile":        parsed.get("phone") or "N/A",
